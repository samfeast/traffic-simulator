...
namespace simulator
{
RoadSegment::RoadSegment(const HorizontalProfile& horizontal, const VerticalProfile& vertical, const Vector3& origin)
    : horizontal_(horizontal), vertical_(vertical), origin_(origin)
{
...
double RoadSegment::getLength() const { return vertical_.length(); }

Vector3 RoadSegment::getOrigin() const { return origin_; }
...
RoadPose RoadSegment::evaluate(double s) const
{
    ...
    return RoadPose{origin_ + Vector3{horizontalPose.xy.x, horizontalPose.xy.y, verticalPose.z}};
}
}  // namespace simulator